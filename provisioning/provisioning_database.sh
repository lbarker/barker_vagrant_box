...
DBASE_FILES=/var/www/barkers_vagrant_box/provisioning/databases/*.sql
...
do
    ...
    then
        ...
        # load data
        mysql -uroot -proot "$DATABASE_NAME" < /var/www/barkers_vagrant_box/provisioning/databases/"$DATABASE_NAME".sql 2> /dev/null
    fi
done